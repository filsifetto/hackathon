...
Configuration is loaded from the **hackathon repo root** `.env` first (so RAG uses the same `OPENAI_API_KEY` as the rest of the project, e.g. avatar). You can override with a `.env` in `src/rag/` if needed. Create or copy `.env.example`:
