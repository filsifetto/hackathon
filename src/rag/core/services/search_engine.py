...
class HybridSearchEngine:
    ...
    async def search(
        ...
        sorted_by_score = sorted(hybrid_results, key=lambda x: x.combined_score, reverse=True)
        # If nothing passes the threshold, return top results anyway so user sees something
        final_results = (
            sorted(filtered_results, key=lambda x: x.combined_score, reverse=True)[:limit]
            if filtered_results
            else sorted_by_score[:limit]
        )
