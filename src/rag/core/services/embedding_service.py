...
class EmbeddingService:
    ...
    def _should_reduce_batch(self, e: Exception) -> bool:
        """True if error indicates we should retry with a smaller batch (429, 400 token limit, or request too large)."""
        err_str = str(e).lower()
        ...
        if "413" in err_str:  # HTTP Payload Too Large
            return True
        # OpenAI 400: "Requested X tokens, max 300000 tokens per request" (max_tokens_per_request)
        if "max_tokens_per_request" in err_str or ("300000" in err_str and "tokens" in err_str and "request" in err_str):
            return True
        return False
