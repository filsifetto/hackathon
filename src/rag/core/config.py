...
# RAG package root (src/rag)
PROJECT_ROOT = Path(__file__).resolve().parent.parent
# Hackathon repo root — load .env from here so RAG uses the same API keys as the rest of the project
REPO_ROOT = PROJECT_ROOT.parent.parent

# Env files: repo root first (shared OPENAI_API_KEY etc.), then local so src/rag/.env can override
_ENV_FILES = (str(REPO_ROOT / ".env"), ".env")
...
class Settings(BaseSettings):
    ...
    min_search_score: float = Field(0.25, env="MIN_SEARCH_SCORE")
    ...
    batch_size: int = Field(300, env="BATCH_SIZE")
